...
class AvlTree{
    ...
    inOrderPlegable(){
        let html = this.#inOrderRecursivePlegable(this.root);
        return html;
    }
    #inOrderRecursivePlegable(current){
        let row = "";
        if(current.left != null){
            row += this.#inOrderRecursivePlegable(current.left);
        }
        row +=`<option value="${current.item.carnet}">${current.item.carnet}</option>`;
        if(current.right != null){
            row += this.#inOrderRecursivePlegable(current.right);
        }
        return row;
    }
    inOrderPlegable2(){
        let html = this.#inOrderRecursivePlegable2(this.root);
        return html;
    }
    #inOrderRecursivePlegable2(current){
        let row = "";
        if(current.left != null){
            row += this.#inOrderRecursivePlegable2(current.left);
        }
        row +=`<option value="${current.item.carnet}">${current.item.nombre}</option>`;
        if(current.right != null){
            row += this.#inOrderRecursivePlegable2(current.right);
        }
        return row;
    }

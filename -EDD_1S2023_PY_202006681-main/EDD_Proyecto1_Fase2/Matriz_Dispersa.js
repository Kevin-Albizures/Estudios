...
class SparseMatrix{
    ...
    constructor(){
        this.head =  new Mnode("-1", "-1", "DOCUMENTOS");
    }
    
    

    insertMatriz(x, y, value) {
    ...
    #xHeaders(x){
        ...
        if(this.head.down == null){
            ...
            while(temp.down != null || temp.x==curr.value){
                temp = temp.down;
            }
            if (temp.x!=curr.value){
                ...
                console.log("cabecera en x ya insertada")
            }
        }
    }
    
    #yHeaders(y){
        let curr = new Mnode("-1","-1", y);
        if(this.head.right == null){
            ...
            // ENCONTRAR EL ESPACIO PARA LA CABECERA
            while(temp.right != null || temp.y==curr.value){
                temp = temp.right;
            }
            //INSERTAR AL FINAL SI ES ULTIMO
            if (temp.y!=curr.value){
                if(temp.right == null){
                    temp.right = curr;
                    curr.left = temp;
                    console.log("cabecera insertada")
    ...
    graph3(temp){
        ...
        code += this.#headersGraph(temp.right)
        code += this.#nodesGraph(temp)
        console.log(code)
        return code
    }
    #headersGraph(temp){
